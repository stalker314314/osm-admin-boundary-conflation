...
def main():
    # Load opstine
    municipalities = {}  # maps (district, municipality) -> maticni broj
    with open('opstina.csv') as opstine_csv:
        reader = csv.DictReader(opstine_csv)
        for row in reader:
            municipalities[(row['okrug_ime'], row['opstina_ime'])] = row['opstina_maticni_broj']

    # Load saved state obtained in measure_quality.py
    progress_file = 'output/measure_quality_opstine.csv'
    results = []
    ...
    api = osmapi.OsmApi(passwordfile='osm-password',
                        ...
                        {u"comment": u"Serbian lint bot - adding missing ref:RS:opstina"
    ...
    for i, result in enumerate(results):
        maticni_broj = municipalities[(result['district'], result['municipality'])]
        ...
        accepted = False
        while True:
            response = input('({0}/{1}) Are you sure you want to add ref:RS:opstina {2} to {3}/{4} (Y/n/c)?'.format(
        ...
        if not accepted:
            continue
        relation['tag']['ref:RS:opstina'] = maticni_broj
